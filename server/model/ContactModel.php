...
class ContactModel {
    ...
    public function create($data , $allowedKeys = []) {
        ...
        $query = "INSERT INTO CONTACT (" . implode(", ", $keys) . ") VALUES ('" . implode("', '", $values) . "')";
    ...
    public function read($queryParams, $allowedKeys = [], $select = []) {
        ...
        $query = "SELECT $selectClause FROM CONTACT $whereClause";
    ...
    public function update($id, $data, $allowedKeys = []) {
        ...
        $query = "UPDATE CONTACT SET " . implode(", ", $updates) . " WHERE id='$id'";
        if ($this->connection->query($query)) {
            return true;
        } else {
            return false;
